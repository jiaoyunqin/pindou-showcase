...
本轮实际采用**方向 A + 方向 B 实证对照**：以“拼豆图纸说明册”为主视觉方向；八章横向章节顶栏已按用户反馈删除，章节由 eyebrow、标题、背景分区和内容构图区分，猫咪案例继续通过图片、图注和正文串联核心流程。首屏增加猫咪原图和小程序码，独立案例保留“原始生成图片 → 导出图纸”对应关系。因与导出图高度重复，应用内生成画面组已按用户意见删除。未实施方向 D 实物展示。
...
- 八章横向章节顶栏已按用户反馈删除；章节由 eyebrow、标题、背景分区和内容构图区分；全局导航和 8 个章节锚点保持可用，标题可见且不被导航遮挡。
- 31 张正文图均返回 WebP、自然宽度有效；31 个灯箱入口均可打开、Esc 关闭并返回焦点。
- 灯箱关闭按钮、遮罩、焦点循环、原始尺寸模式和公开来源链接均通过。
- 标题、截图和内容区的一次性轻量显现、灯箱与投稿详情展开等渐进交互保留；无 JavaScript、`prefers-reduced-motion: reduce`、`file://` 直接打开、本地 HTTP 服务均通过。
